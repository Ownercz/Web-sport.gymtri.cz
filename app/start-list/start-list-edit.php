...
<div class='container theme-showcase' role='main'>
    ...
    <table class='table table-hover' style='margin-bottom:0px;'>
        ...
        <strong>Vybráno!</strong> Závod byl úspěšně vybrán a nyní je potřeba vybrat disciplínu k edtitaci startovky! (pokud byl vybrán špatný závod, klepněte <a href='?'>zde</a>)
            ...
            $i = 0;$i++;//For not to be seen weird.
            ...
            while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
                ...
                $sex = $row["gender"];
                $age = $row["age"];
                echo "<form action='start-list-edit-script.php?id=" . $id . "&athleteid=" . $row["athlete_id"] . "' method='POST' target='_blank' onsubmit='setTimeout(function () { window.location.reload(); }, 30)'>
                ...
                echo"</td>
          <td class='class'>".eventAgeClass($id,$athlete["id"],$mysqli).".".$athlete["class"]."</td><td class='birth'>".$athlete["birthdate"]." | Věk: ".$row["age"];
