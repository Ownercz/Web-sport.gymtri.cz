...
/*athlete info*/
 ...
 while($row1 = $result1->fetch_array(MYSQLI_NUM)){$jmeno = $row[2]; $prijmeni = $row[3];}
    $request2 ="SELECT * FROM `event` WHERE `id` = $id ";
    $result2 = $mysqli->query($request2);
    while($row2 = $result2->fetch_array(MYSQLI_NUM)){$event_name=$row[2]; $event_date = $row[4];}
