...
if (isset($_GET['id'])) {
    ...
    if ($result->num_rows === 0) {
        ...
        while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
          ...
          <td class='class'>" . eventAgeClass($event_id, $athlete["id"], $mysqli) . "." . $athlete["class"] . "</td><td class='birth'>" . $athlete["birthdate"] . " | Věk: " . $row["age"] . "</td>";
            ...
            echo "<td class='class'><input type='text' name='vykon' placeholder='" . rand(0, 59) . "," . rand(0, 59) . "'></td><td><input type='submit' class='btn btn-sm btn-default' value='Vložit'></td></form>";
        }
        echo "</tbody></table>";
  ...
  <span class='glyphicon glyphicon-trash' aria-hidden='true'></span> Odstranit
   ...
   echo"</select></li> <li class='score'><input type='text' name='vykon' size='10'></input></li><li class='save'><input type='submit' class='btn btn-sm btn-default' value='Uložit'></input></li></ul></form>
    ...
    $request = "SELECT * FROM  `event_score`  WHERE  `score_value` IS NOT NULL  AND  `score_points` IS NOT NULL  AND  `koeficient` IS NOT NULL AND `event_id` = '$event_id' AND `discipline_id` = '$discipline_id'"; //AND `yearbegin` = '$beginyear'"  ; NO USED QUERY
    $result = $mysqli->query($request);
    //echo $classid;
    if ($result->num_rows === 0) {
        echo "<button type='button' class='btn btn-warning' style='width:100%;text-align:center;'>Zatím jste nevložili žádné výsledky, až je vložíte, zobrazí se pod tabulku závodníků.</button>";
    } else {
